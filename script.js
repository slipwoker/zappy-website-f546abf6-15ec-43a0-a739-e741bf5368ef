document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    const VALIDATORS = {
      ...
      minlength: (value, attr) => value.trim().length >= parseInt(attr, 10)
    };
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required') || field.required) {
        if (!VALIDATORS.required(field.type === 'checkbox' ? value : field.value)) {
      ...
      // Tel validation
      if (field.type === 'tel' && field.value.trim() && !VALIDATORS.tel(field.value)) {
        showError(field, getErrorMessage(field, 'tel'));
        return false;
      }

      // Minlength validation
      if (field.hasAttribute('minlength') && !VALIDATORS.minlength(field.value, field.getAttribute('minlength'))) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'f546abf6-15ec-43a0-a739-e741bf5368ef',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'f546abf6-15ec-43a0-a739-e741bf5368ef';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f546abf6-15ec-43a0-a739-e741bf5368ef';
